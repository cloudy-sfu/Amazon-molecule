...
for mass, mass, mass_height in mass_heights:
...
for mass, cols_this_height in cols_grouped_mass.items():
    ...
    for i, j in product(range(x_train.shape[2]), repeat=2):
        if i == j:
            continue
        if os.path.exists(f'raw/9_{dataset_name}_nn_{lag}/{mass}_{i}_{j}_r.h5'):
        ...
        sbm_ur = tf.keras.callbacks.ModelCheckpoint(f'raw/9_{dataset_name}_nn_{lag}/{mass}_{i}_{j}_ur.h5',
                                                    save_best_only=True)
        ...
        sbm_r = tf.keras.callbacks.ModelCheckpoint(f'raw/9_{dataset_name}_nn_{lag}/{mass}_{i}_{j}_r.h5',
                                                   save_best_only=True)
